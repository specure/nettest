...
fn test_handle_put_1() {
    ...
    rt.block_on(async {
        ...
        // Отправляем PUT команду
        let chunk_size = 4096; // 4KB
        debug!("Sending PUT command: chunk_size={}", chunk_size);
        let put_cmd = format!("PUT {}\n", chunk_size);
        debug!("PUT command bytes: {:?}", put_cmd.as_bytes());
        stream.write_all(put_cmd.as_bytes())
            .await
            .expect("Failed to send PUT");
        stream.flush().await.expect("Failed to flush PUT command");
        debug!("PUT command sent and flushed");
        ...
        for i in 0..chunks {
            ...
            println!("Chunk data (first 32 bytes): {:?}", &chunk[..std::cmp::min(32, chunk_size)]);
            trace!("Chunk data (last 32 bytes): {:?}", &chunk[chunk_size.saturating_sub(32)..chunk_size]);
            trace!("Last byte: 0x{:02X}", chunk[chunk_size - 1]);
            
            // Ждем немного, чтобы дать серверу время на отправку TIME BYTES
        ...
        assert_eq!(total_bytes, chunks * chunk_size, 
                  "Should send exactly {} chunks of {} bytes each", 
                  chunks, chunk_size);
        
        // Ждем финальный TIME ответ от сервера
        let mut response = [0u8; 1024];
        let n = stream.read(&mut response).await.expect("Failed to read final TIME response");
        let time_response = String::from_utf8_lossy(&response[..n]);
        debug!("Received final TIME response: {}", time_response);
