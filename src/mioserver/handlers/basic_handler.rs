...
pub fn handle_client_readable_data(state: &mut TestState, poll: &Poll) -> io::Result<usize> {
    match state.measurement_state {
        ...
        ServerTestPhase::SignedResultReceiveOk => handle_signed_result_receive_ok(poll, state),
...
pub fn handle_client_writable_data(state: &mut TestState, poll: &Poll) -> io::Result<usize> {
    match state.measurement_state {
        ...
        ServerTestPhase::PutTimeResultSendInterim => handle_put_time_result_send_interim(poll, state),
        ServerTestPhase::PutTimeResultSendTimeResult => handle_put_time_result_send_time(poll, state),

        ServerTestPhase::SignedResultSend => handle_signed_result(poll, state),

        ServerTestPhase::VoipSendOk => handle_voip_send_ok(poll, state),
        ServerTestPhase::VoipSendResult => handle_voip_send_result(poll, state),

        ServerTestPhase::UdpSendPort => handle_udp_send_port(poll, state),
        ServerTestPhase::UdpSendOkOut => handle_udp_send_ok_out(poll, state),
