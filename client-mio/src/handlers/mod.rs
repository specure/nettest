...
pub async fn handle_greeting(stream: &mut TcpStream, is_websocket: bool) -> Result<(), Box<dyn Error + Send + Sync>> {
...
pub async fn handle_quit(stream: &mut TcpStream) -> Result<(), Box<dyn Error + Send + Sync>> {
    // Send BYE to client
